...
foreach ($detailPanier as $idArticle => $idDetail) {
    // print_r($detailPanier[$idArticle]['prix']);

	// oupsi
	$totalPrix += $idDetail["prix"]*$idDetail["qte"];
	//print_r($totalPrix);
	$totalPanier += $idDetail['qte'];

}

$panierSansRien= "none";
//partie pour l'etape 8 qui est  le badge

if( count($detailPanier) > 0){
	$panierSansRien="block";
}
...
//variable qui va etre utiliser pour appeler les info dans le panierSommaire qui se 
//trouve dans l'internationalisation 

$_sommaire= $textes["panierSommaire"];
		...
		<header>
			...
			<nav class="barre-logo">
				...
				<div class="panier-icone">
					...
					<div class="badge" style="display: <?=$panierSansRien ;?>"> <?= $totalPanier; ?></div>
					...
					<div class="sommaire-panier">
						<div class="ligne1">
							<span class="nb-articles">
								<span class="etiquette">#<?=$_sommaire["articles"] ;?> : </span>
								<span class="nombre"><?= $totalPanier; ?></span>
							</span>
							<label for="panier-cc" class="material-icons">close</label>
						</div>
						<div class="ligne2">
							<span class="sous-titre"><?= $_sommaire["sousTotal"]; ?></span>
							<span class="sous-total montant-fr"><?= number_format($totalPrix, 2); ?></span>
						</div>
						<div class="ligne3 btn-afficher-panier">
							<a href="panier.php"><?= $_sommaire["voirPanier"]; ?></a>
						</div>
					</div>
				</div>
